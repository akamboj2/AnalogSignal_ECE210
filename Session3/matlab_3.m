...
plot(tout,di_out);
xlabel('time t');
ylabel('rate of change of current i');
hold off;
...
for i=N
    pi_es=pi_es+1/(i^2);
    pi_estimate(i)=pi_estimate(i-1)+1/(i^2);
end
...
error=ones(1,length(N)+1)*pi^2/6;
error=error-pi_estimate;
size(error)
size(pi_estimate)
plot(1:100,error);
error
